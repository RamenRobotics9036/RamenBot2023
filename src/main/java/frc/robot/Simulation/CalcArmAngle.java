...
/*
 * We make some simplifying assumptions for the simulation: 1) Hardcoded length of arm, and distance
 * from winch to point that string is attached to arm 2) Assume that the point that string is
 * attached to arm can slide up and down the arm, to ensure that the string is always perpendicular
 * to the floor. This isn't ideal, but simplifies the math as it ensures there's always a right
 * triangle :) Reminder: 0 degrees points to the right, 90 straight up, 180 to left, 270 down
 */
...
public class CalcArmAngle {
  ...
  public CalcArmAngle(double heightFromWinchToPivotPoint, double armLengthFromEdgeToPivot) {
    if (heightFromWinchToPivotPoint < Constants.SimConstants.karmHeightFromWinchToPivotPoint_Min) {
      throw new IllegalArgumentException("Height from winch to arm needs to be at least "
          + Constants.SimConstants.karmHeightFromWinchToPivotPoint_Min + " meters");
    }
  ...
  public Result GetDegreesForStringLength(double stringLen) {
    ...
    if (backOfArmHeightAbovePivot == 0) {
      ...
      return new Result(true, 360
          - RightTriangleAngleGivenHeight(m_armLengthFromEdgeToPivot, backOfArmHeightAbovePivot));
    }
    else {
      return new Result(true, RightTriangleAngleGivenHeight(m_armLengthFromEdgeToPivot,
          Math.abs(backOfArmHeightAbovePivot)));
    }
  }
